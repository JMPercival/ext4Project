...
self.s_uuid[16]= int(getHex(self.superblock, 0x68, 0x69, False), 16)#128-bit UUID for volume.
self.s_volume_name[16]= int(getHex(self.superblock, 0x78, 0x79, False), 16)#Volume label.
self.s_last_mounted[64]= int(getHex(self.superblock, 0x88, 0x89, False), 16)#Directory where filesystem was last mounted.
...
self.s_journal_uuid[16]= int(getHex(self.superblock, 0xD0, 0xd1, False), 16)#UUID of journal superblock
...
self.s_hash_seed[4]= int(getHex(self.superblock, 0xEC, 0xf0, True), 16)#HTREE hash seed.
...
self.s_jnl_blocks[17]= int(getHex(self.superblock, 0x10C, 0x110, True), 16)#None
...
self.s_first_error_func[32]= int(getHex(self.superblock, 0x1A8, 0x1a9, False), 16)#Name of function where the error happened.
...
self.s_last_error_func[32]= int(getHex(self.superblock, 0x1E0, 0x1e1, False), 16)#Name of function where the most recent error happened.
self.s_mount_opts[64]= int(getHex(self.superblock, 0x200, 0x201, False), 16)#ASCIIZ string of mount options.
...
self.s_backup_bgs[2]= int(getHex(self.superblock, 0x24C, 0x250, True), 16)#Block groups containing superblock backups (if sparse_super2)
self.s_encrypt_algos[4]= int(getHex(self.superblock, 0x254, 0x255, False), 16)#None




self.s_encrypt_pw_salt[16]= int(getHex(self.superblock, 0x258, 0x259, False), 16)#Salt for the string2key algorithm for encryption.
...
self.s_reserved[99]= int(getHex(self.superblock, 0x270, 0x274, True), 16)#Padding to the end of the block.
